...
import { requestGetViewBook } from '../../config/request';
...
const ViewedProducts = () => {
    ...
    useEffect(() => {
        const fetchViewBooks = async () => {
            const res = await requestGetViewBook();
            setViewedBooks(res.metadata);
        };
        fetchViewBooks();
    }, []);
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-2xl font-semibold mb-6 text-gray-800">Sách đã xem</h2>

            {viewedBooks.length === 0 ? (
                <div className="text-center py-10">
                    <Empty description="Bạn chưa xem sách nào" />
                </div>
            ) : (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    {viewedBooks.map((book) => (
                        <CardBody product={book.book} />
                        // <div
                        //     key={product._id}
